...
const replaceVerticalLinesForTable = parsedString => {
...
const addNewLineForComplexObject = (descrTag) => {
    ...
    for (const objMatch of objMatches) {
        ...
        const propMatches = [...objMatch[0].matchAll(/((((\w+|\\?\"[\w\§\$\%\&\(\)\=\:\@\[\]]+\\?\")(\?\:|\:)\w+)|((\[\w+|\\?\"[\w\§\$\%\&\(\)\=\:\@\[\]]+\\?\")(\?\:|\:)\w+)\](\?\:|\:)\w+)\;)/g)];
        if (propMatches !== null) {
            outStr += "{";
            for (const [ipropMatch, propMatch] of propMatches.entries()) {
                ...
                outStr += pText;
                if (propMatches.length > 1 && ipropMatch !== propMatches.length - 1)
                    outStr += "<br/>"
            }
            outStr += "}";
        }
...
const addNewLineForComplexFunctions = (typeTag) => {
    ...
    for (const typeStrFuncMatch of typeStrFuncMatches) {
        ...
        const typeStrFuncParMatches = [...typeStrFuncMatch[0].matchAll(/((((\w+|\\?\"[\w\§\$\%\&\(\)\=\:\@\[\]]+\\?\")(\?\:|\:)\w+)|((\[\w+|\\?\"[\w\§\$\%\&\(\)\=\:\@\[\]]+\\?\")(\?\:|\:)\w+)\](\?\:|\:)\w+)\,?)\,?/gm)]
        if (typeStrFuncParMatches !== null) {
            outStr += "(";
            for (const [itypeStrFuncParMatch, typeStrFuncParMatch] of typeStrFuncParMatches.entries()) {
                const txt = typeTagNoWhiteSp.substr(iStartTypeStrFuncMatch + typeStrFuncParMatch.index, typeStrFuncParMatch[0].length)
                outStr += txt;
...
const parseDocument = (file) => {
    ...
    for (docElement of dg) {
        if ("docu" in docElement.tags) {
            ...
            if ("displayName" in docElement)
                elName = docElement.displayName;
            else if ("description" in docElement)
                elName = docElement.description;
            else
                console.log("WARNING: element's displayname or description not found.", docElement);
            let elRemark = "";
            if ("tags" in docElement) {
                if ("dummy" in docElement.tags) {
                    if (typeof docElement.tags.dummy === "string") {
                        if (docElement.tags.dummy.trim().length > 0)
                            elName = docElement.tags.dummy.trim();
                    }
                }
            ...
            if ("props" in docElement) {
                if (Object.keys(docElement.props).length >= 1) {
                    PropTupleArray = Object.entries(docElement.props).sort(Comparator2);
                ...
                for (const [propKey, propVal] of PropTupleArray) {
                    ...
                    if ("defaultValue" in propVal)
                        if (propVal.defaultValue !== null)
                            if ("value" in propVal.defaultValue)
                                prop_DefaultValue = propVal.defaultValue.value;
                    ...
                    //if ("tags" in propVal)
                    //    if (propVal.tags !== null) {
                    //        propTags = propCustomObj;//propVal.tags;
                    if ("internal" in propTags)
                        isInternal = true;
                    ...
                    if ("desc" in propTags)
                        if (typeof propTags.desc === "string")
                            if (propTags.desc.length > 0)
                                useDescTagAsDescription = true;
                    //  }
                    ...
                    if (!isInternal) {
                        let propName = replaceVerticalLinesForTable(quoteAngleBracket(propKey));
                        mdPropTable += `| [${propName}](#${propName}) | ${replaceVerticalLinesForTable(prop_Type)} | ${replaceVerticalLinesForTable(prop_DefaultValue)} | ${propRequired} | ${replaceVerticalLinesForTable(propShortDescription)} |\r\n`;
                        mdPropDetails += "### \\(`" + quoteAngleBracket(propKey) + "`\\)" + propRequiredTag + " \r\n"
                            + propFullDescription
                    }
                }
...
const loopDocs = (dir, dest) => {
    ...
    for (dirFile of dirFiles) {
        ...
        const fileExt = dirFile.substr(dirFile.lastIndexOf("."));
        if (isFolder)
            loopDocs(dir + "/" + dirFile, dest);
        else if (fileExt === ".tsx" || fileExt === ".ts") {
...
if (cliArgs.length === 0) { // no cli arg provided 
