...
export async function csvfile(
  ...
  skipEmptyLines = true,
  preview = 0
): Promise<csvResultTransformedType> {
...
const AssignDataColumnKeys = (data: string[][], keys: string[]): csvDataObjectified[] => {
  ...
  data.forEach((datarow, j) => {
    ...
    datarow.forEach((singledata, i) => {
      if (i < keys.length) rowobj[keys[i]] = singledata;
      else rowobj[i] = singledata;
    });
    rowobj.id = j;
